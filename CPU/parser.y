...
%token Y_GT_MACRO
%token Y_GTE_MACRO
%token Y_FSTART_MACRO
%token Y_FEND_MACRO
%token Y_KSTART_MACRO
%token Y_KEND_MACRO
%token Y_CFUNC_MACRO
%token Y_CFUNCEND_MACRO
%token Y_CCALL_MACRO
...
#include <stdio.h>
#include <math.h>
#include <unistd.h>   /*CS296*/
#include <sys/wait.h> /*CS296*/
...
#define YYDEBUG 1
#define MAX_NUM_FUNCS 100 /*CS296*/
...
static int num_funcs = 0;       /*CS296: number of cfunctions declared */

static char * function_names[MAX_NUM_FUNCS];  /*CS296: list of names of cfunctions */
...
MACROS:		MACRO_OPERAND1 Y_EQ_MACRO MACRO_OPERAND2
 ...
 |      MACRO_OPERAND1 Y_EQ_MACRO MACRO_OPERAND2C
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7);
			assert_msg[0] = '\0';
			sprintf(assert_msg, "\n%s$%d == %d\n %d \n", assert_failed, $1.i, $3.i);
			...
			int cur_data_pc = current_data_pc();
			store_string(assert_msg, strlen(assert_msg), 1);
            i_type_inst(Y_ORI_OP, 1, 0, make_imm_expr($3.i, NULL, 0));
			i_type_inst(Y_BEQ_OP, $1.i, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
 ...
 |      MACRO_OPERAND1 Y_NEQ_MACRO MACRO_OPERAND2C
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
			...
			store_string(assert_msg, strlen(assert_msg), 1);
            i_type_inst(Y_ORI_OP, 1, 0, const_imm_expr($3.i));
			i_type_inst_free(Y_BNE_OP, $1.i, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
  ...
  |     MACRO_OPERAND1 Y_LTE_MACRO MACRO_OPERAND2
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) log($3.i) + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
			...
			store_string(assert_msg, strlen(assert_msg), 1);
            r_type_inst(Y_SUB_OP, 1, $1.i, $3.i);
			i_type_inst(Y_BLEZ_OP, 1, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
  ...
  |     MACRO_OPERAND1 Y_LT_MACRO MACRO_OPERAND2
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) log($3.i) + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
			...
			store_string(assert_msg, strlen(assert_msg), 1);
            r_type_inst(Y_SUB_OP, 1, $1.i, $3.i);
			i_type_inst(Y_BLTZ_OP, 1, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
  ...
  |     MACRO_OPERAND1 Y_LTE_MACRO MACRO_OPERAND2C
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
  ...
  |     MACRO_OPERAND1 Y_LT_MACRO MACRO_OPERAND2C
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
  ...
  |     MACRO_OPERAND1 Y_GTE_MACRO MACRO_OPERAND2
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) log($3.i) + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
			...
			store_string(assert_msg, strlen(assert_msg), 1);
            r_type_inst(Y_SUB_OP, 1, $1.i, $3.i);
			i_type_inst(Y_BGEZ_OP, 1, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
  ...
  |     MACRO_OPERAND1 Y_GT_MACRO MACRO_OPERAND2
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) log($3.i) + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
			...
			store_string(assert_msg, strlen(assert_msg), 1);
            r_type_inst(Y_SUB_OP, 1, $1.i, $3.i);
			i_type_inst(Y_BGTZ_OP, 1, 1, const_imm_expr(30));
			i_type_inst(Y_ADDI_OP, 4, 0, const_imm_expr(cur_data_pc));
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(4));
			r_type_inst(Y_SYSCALL_OP, 0, 0, 0);
			i_type_inst(Y_ORI_OP, 2, 0, const_imm_expr(10));
  ...
  |     MACRO_OPERAND1 Y_GTE_MACRO MACRO_OPERAND2C
        {
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
  ...
  |     MACRO_OPERAND1 Y_GT_MACRO MACRO_OPERAND2C
        { 
			int size1 = (int) log($1.i) + 2, size2 = (int) 20 + 2;
			char *assert_failed = "Assertion failed: ";
			char *assert_msg = malloc(strlen(assert_failed) + size1 + size2 + 7 + 100);
  ...
  |     Y_FSTART_MACRO
        {
            i_type_inst(Y_FSTART_MACRO, 0, 0, const_imm_expr(0));
            LINE_PARSE_DONE;
        }
  |     Y_FEND_MACRO
        {
            i_type_inst(Y_FEND_MACRO, 0, 0, const_imm_expr(0));
            LINE_PARSE_DONE;
        }
  |     Y_KSTART_MACRO
        {
            i_type_inst(Y_KSTART_MACRO, 0, 0, const_imm_expr(0));
            LINE_PARSE_DONE;
        }
  |     Y_KEND_MACRO
        {
            i_type_inst(Y_KEND_MACRO, 0, 0, const_imm_expr(0));
            LINE_PARSE_DONE;
        }
  |     Y_CFUNC_MACRO Y_STR Y_STR
        {

            char * func_name = $2.p;
            char * func = (char*) $3.p;
            char * print_string = malloc(sizeof(char) * 255);
            char * exename = malloc(strlen("cfunc") + sizeof(char)*12);

            //make fprintf line
            strcpy(print_string, "fprintf(ret, \"%d\",");
            strcat(print_string, func_name);
            strcat(print_string, "(a0, a1, a2, a3));\n");

            //make executable file name
            strcpy(exename, "cfunc");
            char * numstr = malloc(sizeof(char) * 20);
            sprintf(numstr, "%d", num_funcs);
            strcat(exename, numstr);
            free(numstr);

            //fill c file with boilerplate code
            FILE * func_file;
            if((func_file = fopen("func_file.c", "w+")) == NULL)
            {
            ...
            fputs("int a0 = atoi(argv[1]), a1 = atoi(argv[2]), a2 = atoi(argv[3]), a3 = atoi(argv[4]);\n", func_file);
            fputs("FILE * ret = fopen(\"return.out\", \"w+\"); \n", func_file);
            fputs(print_string, func_file);
            fputs("fclose(ret);\n", func_file);
            fputs("return 0;\n", func_file);
            fputs("}\n", func_file);
            fputs(func, func_file);
            fclose(func_file);
            ...
            if(pid == 0)
            {
                execlp("gcc", "gcc", "func_file.c", "-o", exename, (char*) 0);
                printf("comp failed for file %s \n", func_name);
                exit(1);
            }
            else
            {
            ...
            //update function indexing structure
            char * temp = malloc(strlen(func_name));
            strcpy(temp, func_name);
            function_names[num_funcs] = temp;
            num_funcs++;
            if(num_funcs > MAX_NUM_FUNCS)
            {
                printf("too many c functions, write some MIPS! \n");
                exit(1);
            }
            remove("func_file.c");
            LINE_PARSE_DONE;
  ...
  |     Y_CCALL_MACRO Y_STR
        {
            int index;
            int flag = 0;
            char * func_name = (char*) $2.p;

            //walk through function names looking for this one
            for(index = 0; index < num_funcs; index++)
            {
                if(strcmp(function_names[index],func_name) == 0)
                {
                    flag = 1;
                    break;
                }
            ...
            if(!flag)
            {
                printf("ccall: function %s not declared \n", func_name);
                exit(1);
            }
                
            i_type_inst(Y_CCALL_MACRO, index, 0, const_imm_expr(0));
            LINE_PARSE_DONE;
        }
        ;

 ...
 */
MACRO_OPERAND2: Y_REG ;
MACRO_OPERAND2C: Y_INT;
