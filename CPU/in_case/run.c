      ...
      for (step = 0; step < step_size; step += 1)
	{
	  ...
	  switch (OPCODE (inst))
	    ...
	    case Y_BC2F_OP:
        {
            num_nests++;
            int index;
            fstart = malloc(sizeof(int) * 10);
            //store s registers
            for(index = 16; index <= 23; index++)
                fstart[index-16] = R[index];
	    ...
	    case Y_BC2FL_OP:
        {
                ...
                for(index = 0; index < 8; index++)
                    if(R[index+16]!=fstart[index])
                        printf("Assertion Failed: You modified a $s register");

                //confirm sp register unchanged
                if(fstart[8]!= R[29]) printf("Assertion Failed: You modified the stack pointer");

                //confirm $ra unchanged
                if(fstart[9]!=R[31]) printf("Assertion Failed: You modified $ra");
                free(fstart);
        }
