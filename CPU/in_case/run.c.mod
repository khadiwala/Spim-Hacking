  ...
  for (step_size = MIN (next_step, steps_to_run);
      ...
      for (step = 0; step < step_size; step += 1)
	{
	  ...
	  switch (OPCODE (inst))
	    ...
	    case Y_BC2F_OP:
        {
            printf("num_nests prior to call %d \n", num_nests);
            num_nests += 1;
            printf("num_nests after the call %d \n", num_nests);
            int index;
            fstart = realloc(fstart, sizeof(int) * 10 * (num_nests));
            //store s registers
            for(index = 16; index <= 23; index++)
                fstart[(num_nests)*10 + index-16] = R[index];
   
            //store sp
            fstart[(num_nests)*10 + 8] = R[29];
  
            //store ra
            fstart[(num_nests)*10 + 9] = R[31];
            break;
        }
        ...
        {
                num_nests-=1;
                printf("num_nests in the closeur %d \n", num_nests);
                int index;

                //confirm s registers unchanged
                for(index = 0; index < 8; index++)
                    if(R[index+16]!=fstart[(num_nests)*10 + index])
                        printf("Assertion Failed: You modified a $s register \n");

                //confirm sp register unchanged
                if(fstart[(num_nests)*10 + 8]!= R[29]) printf("Assertion Failed: You modified the stack pointer \n");

                //confirm $ra unchanged
                if(fstart[(num_nests)*10 + 9]!=R[31]) printf("Assertion Failed: You modified $ra \n");
                //if(num_nests==0)
                   // free(fstart);
        }
