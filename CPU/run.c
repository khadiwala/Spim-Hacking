...
run_spim (mem_addr initial_PC, int steps_to_run, int display)
{
  ...
  /*CS296 variable*/
  static int * fstart = NULL;
  static int num_nests = 0;
  static int * kstart = NULL;
      ...
      for (step = 0; step < step_size; step += 1)
	{
	  ...
	  switch (OPCODE (inst))
        ...
        case Y_FSTART_MACRO:
        {
            ...
            if(num_nests > 500)
            {
                printf("Over 500 function calls, don't use f_start \n");
                exit(1);
            } 
            int index;
            fstart = realloc(fstart, sizeof(int) * 10 * (num_nests));
            int start = (num_nests-1)*10;
            //store s registers
            for(index = 16; index <= 23; index++)
                fstart[start + index-16] = R[index];
        ...
        case Y_FEND_MACRO:
        {
                if(fstart == NULL)
                {
                    printf("f_end present with no f_start");
                    exit(0);
                }

                num_nests--;
                int start = num_nests * 10;
                int index;

                //confirm s registers unchanged
                for(index = 0; index < 8; index++)
                    if(R[index+16]!=fstart[start + index])
                    {
                        printf("Assertion Failed: You modified a $s register, register - %d \n", index);
                        exit(0);
                    }

                //confirm sp register unchanged
                if(fstart[start + 8]!= R[29])
                {
                    printf("Assertion Failed: You modified the stack pointer \n");
                    exit(0);
                }

                //confirm $ra unchanged
                if(fstart[start + 9]!=R[31])
                {
                    printf("Assertion Failed: You modified $ra \n");
                    exit(0);
                }
                if(!num_nests)
                    free(fstart);
        ...
        {
            int index;
            kstart = malloc(sizeof(int) * 30);

            //store all registers before k0
            for(index = 0; index < 26; index++)
                kstart[index] = R[index];
            
            //store all registers after k1
        ...
        case Y_KEND_MACRO:
        {
            if(kstart == NULL)
            {
                printf("k_end present but no k_start");
                exit(0);
            }

            int index;
            //confirm pre k0 registers intact:
            for(index = 0; index < 26; index++)
                if(R[index] != kstart[index])
                {
                    printf("Assertion Failed: interrupt modified register %d \n", index);
                    exit(0);
                }
            //confirm post k1 registers intact:
            for(index = 28; index < 32; index++)
                if(R[index] != kstart[index])
                {
                    printf("Assertion Failed: interrupt modified register %d \n", index);
        ...
        case Y_CCALL_MACRO:
        {
            ...
            char * a0 = malloc(sizeof(char) * 20);
            char * a1 = malloc(sizeof(char) * 20);
            char * a2 = malloc(sizeof(char) * 20);
            char * a3 = malloc(sizeof(char) * 20);
            sprintf(a0, "%d", R[4]);
            sprintf(a1, "%d", R[5]);
            sprintf(a2, "%d", R[6]);
            sprintf(a3, "%d", R[7]);

            //convert function name index to string
            char * strnum = malloc(sizeof(char) * 20);
            sprintf(strnum, "%d", RT (inst));

            //create string containing executable name
            char * func_name = malloc(strlen("cfunc") + 3*sizeof(char));
            strcpy(func_name, "cfunc");
            strcat(func_name, strnum);

            //create command for executable
            char * exe = malloc(strlen(func_name) + sizeof(char) * 2);
            strcpy(exe, "./");
            strcat(exe, func_name);

            //execute precompiled function
            pid_t pid = fork();
            if(pid == 0)
            {
                execlp(exe, exe, a0, a1, a2, a3, (char*) 0);
                exit(1);
            }
            else
            {
                ...
                {
                    printf("unsucuessfull run of %s\n", func_name);
                    exit(1);
                }
            }

            //set v0 to the return value
            FILE * fs = fopen("return.out", "r");
            char* returnstr = malloc(sizeof(char) * 20);
            R[2] = atoi(fgets(returnstr, 20,fs));
